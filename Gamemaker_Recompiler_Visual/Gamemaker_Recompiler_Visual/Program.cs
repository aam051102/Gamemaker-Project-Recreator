...
namespace Gamemaker_Recompiler_Visual
{
    ...
    {
        public static void Convert_Project_From_Path(string path)
        {
            // FOLDER CREATION
            System.IO.Directory.CreateDirectory(path + "projectA.gmx");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "background");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "Configs");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "datafiles");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "fonts");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "objects");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "paths");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "rooms");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "scripts");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "shaders");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "sound");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "sprites");

            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "sprites\\" + "images");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "background\\" + "images");
            System.IO.Directory.CreateDirectory(path + "projectA.gmx\\" + "sound\\" + "audio");
            ...
            // CONFIGS
            rewrite += "<Configs name=\"configs\">@";

            //rewrite += "    <name>" + "configs" + "</name>@";

            rewrite += "    <Config>" + @"Configs\Default" + "</Config>@";

            rewrite += "</Configs>@";

            // DATAFILES
            rewrite += "<datafiles number=\"0\" name=\"datafiles\">@";

            //rewrite += "    <number>" + "0" + "</number>@";

            //rewrite += "    <name>" + "datafiles" + "</name>@";

            rewrite += "</datafiles>@";

            // EXTENSIONS
            rewrite += "<NewExtensions/>@";

            // SOUNDS
            rewrite += "<sounds name=\"sound\">@";

            //rewrite += "    <name>" + "sound" + "</name>@";

            string[] sounds = Sounds.Get_Files(path + "sounds\\converted\\");
            List<string> actual_sounds = new List<string> { };
            foreach (string sound in sounds)
            {
                if (Path.GetExtension(Path.GetFileName(sound)) == ".gmx")
                {
                    actual_sounds.Add(Path.GetFileNameWithoutExtension(sound));
                    rewrite += "<sound>" + "sound\\" + Path.GetFileNameWithoutExtension(sound).Remove(Path.GetFileNameWithoutExtension(sound).IndexOf(".sound")) + "</sound>" + "@";
                    System.IO.File.Copy(sound, path + "projectA.gmx\\sound\\" + Path.GetFileName(sound));
                }
            }

            string[] sounds_data = Sounds.Get_Files(path + "sounds\\converted\\sound\\audio\\");
            List<string> actual_sounds_data = new List<string> { };
            foreach (string sound_data in sounds_data)
            {
                if (Path.GetExtension(Path.GetFileName(sound_data)) == ".mp3" || Path.GetExtension(Path.GetFileName(sound_data)) == ".wav" || Path.GetExtension(Path.GetFileName(sound_data)) == ".ogg")
                {
                    //actual_sounds.Add(Path.GetFileNameWithoutExtension(font_images));
                    //rewrite += "<font>" + "fonts\\" + Path.GetFileNameWithoutExtension(font_images).Remove(Path.GetFileNameWithoutExtension(font_images).IndexOf(".png")) + "</font>" + "@";
                    System.IO.File.Copy(sound_data, path + "projectA.gmx\\sound\\audio\\" + Path.GetFileName(sound_data));
                }
            }
            ...
            // SPRITES
            rewrite += "<sprites name=\"sprites\">@";

            //rewrite += "    <name>" + "sprites" + "</name>@";

            string[] sprites = Sounds.Get_Files(path + "sprites\\converted\\");
            List<string> actual_sprites = new List<string> { };
            foreach (string sprite in sprites)
            {
                if (Path.GetExtension(Path.GetFileName(sprite)) == ".gmx")
                {
                    actual_sprites.Add(Path.GetFileNameWithoutExtension(sprite));
                    rewrite += "<sprite>" + "sprites\\" + Path.GetFileNameWithoutExtension(sprite).Remove(Path.GetFileNameWithoutExtension(sprite).IndexOf(".sprite")) + "</sprite>" + "@";
                    System.IO.File.Copy(sprite, path + "projectA.gmx\\sprites\\" + Path.GetFileName(sprite));
                }
            }

            string[] sprites_images = Sounds.Get_Files(path + "sprites\\converted\\images\\");
            List<string> actual_sprites_images = new List<string> { };
            foreach (string sprite_image in sprites_images)
            {
                if (Path.GetExtension(Path.GetFileName(sprite_image)) == ".png")
                {
                    //actual_sounds.Add(Path.GetFileNameWithoutExtension(font_images));
                    //rewrite += "<font>" + "fonts\\" + Path.GetFileNameWithoutExtension(font_images).Remove(Path.GetFileNameWithoutExtension(font_images).IndexOf(".png")) + "</font>" + "@";
                    System.IO.File.Copy(sprite_image, path + "projectA.gmx\\sprites\\images\\" + Path.GetFileName(sprite_image));
                }
            }
            ...
            // BACKGROUNDS
            rewrite += "<backgrounds name=\"background\">@";

            //rewrite += "    <name>" + "backgrounds" + "</name>@";

            string[] backgrounds = Sounds.Get_Files(path + "backgrounds\\converted\\");
            List<string> actual_backgrounds = new List<string> { };
            foreach (string background in backgrounds)
            {
                if (Path.GetExtension(Path.GetFileName(background)) == ".gmx")
                {
                    actual_backgrounds.Add(Path.GetFileNameWithoutExtension(background));
                    rewrite += "<background>" + "background\\" + Path.GetFileNameWithoutExtension(background).Remove(Path.GetFileNameWithoutExtension(background).IndexOf(".background")) + "</background>" + "@";
                    System.IO.File.Copy(background, path + "projectA.gmx\\background\\" + Path.GetFileName(background));
                }
            }

            string[] backgrounds_images = Sounds.Get_Files(path + "backgrounds\\converted\\images\\");
            List<string> actual_backgrounds_images = new List<string> { };
            foreach (string background_image in backgrounds_images)
            {
                if (Path.GetExtension(Path.GetFileName(background_image)) == ".png")
                {
                    //actual_sounds.Add(Path.GetFileNameWithoutExtension(font_images));
                    //rewrite += "<font>" + "fonts\\" + Path.GetFileNameWithoutExtension(font_images).Remove(Path.GetFileNameWithoutExtension(font_images).IndexOf(".png")) + "</font>" + "@";
                    System.IO.File.Copy(background_image, path + "projectA.gmx\\background\\images\\" + Path.GetFileName(background_image));
                }
            }
            ...
            // PATHS
            rewrite += "<paths name=\"paths\">@";

            //rewrite += "    <name>" + "paths" + "</name>@";

            rewrite += "</paths>@";

            // SCRIPTS
            rewrite += "<scripts name=\"scripts\">@";

            //rewrite += "    <name>" + "scripts" + "</name>@";

            string[] scripts = Sounds.Get_Files(path + "scripts\\converted\\");
            List<string> actual_scripts = new List<string> { };
            foreach (string script in scripts)
            {
                if (Path.GetExtension(Path.GetFileName(script)) == ".gml")
                {
                    actual_scripts.Add(Path.GetFileNameWithoutExtension(script));
                    rewrite += "<script>" + "scripts\\" + Path.GetFileName(script) + "</script>" + "@";
                    System.IO.File.Copy(script, path + "projectA.gmx\\scripts\\" + Path.GetFileName(script));
                }
            }
            ...
            // SHADERS
            rewrite += "<shaders name=\"shaders\">@";

            //rewrite += "    <name>" + "shaders" + "</name>@";

            rewrite += "</shaders>@";

            // FONTS
            rewrite += "<fonts name=\"fonts\">@";

           // rewrite += "    <name>" + "fonts" + "</name>@";

            string[] fonts = Sounds.Get_Files(path + "fonts\\converted\\");
            List<string> actual_fonts = new List<string> { };
            foreach (string font in fonts)
            {
                if (Path.GetExtension(Path.GetFileName(font)) == ".gmx")
                {
                    actual_fonts.Add(Path.GetFileNameWithoutExtension(font));
                    rewrite += "<font>" + "fonts\\" + Path.GetFileNameWithoutExtension(font).Remove(Path.GetFileNameWithoutExtension(font).IndexOf(".font")) + "</font>" + "@";
                    System.IO.File.Copy(font, path + "projectA.gmx\\fonts\\" + Path.GetFileName(font));
                }
            }

            string[] fonts_images = Sounds.Get_Files(path + "fonts\\converted\\images\\");
            List<string> actual_fonts_images = new List<string> { };
            foreach (string font_images in fonts_images)
            {
                if (Path.GetExtension(Path.GetFileName(font_images)) == ".png")
                {
                    //actual_sounds.Add(Path.GetFileNameWithoutExtension(font_images));
                    //rewrite += "<font>" + "fonts\\" + Path.GetFileNameWithoutExtension(font_images).Remove(Path.GetFileNameWithoutExtension(font_images).IndexOf(".png")) + "</font>" + "@";
                    System.IO.File.Copy(font_images, path + "projectA.gmx\\fonts\\" + Path.GetFileName(font_images));
                }
            }
            ...
            // OBJECTS
            rewrite += "<objects name=\"objects\">@";

            //rewrite += "    <name>" + "objects" + "</name>@";

            string[] objects = Sounds.Get_Files(path + "objects\\converted\\");
            List<string> actual_objects = new List<string> { };
            foreach (string Object in objects)
            {
                if (Path.GetExtension(Path.GetFileName(Object)) == ".gmx")
                {
                    actual_sounds.Add(Path.GetFileNameWithoutExtension(Object));
                    rewrite += "<object>" + "objects\\" + Path.GetFileNameWithoutExtension(Object).Remove(Path.GetFileNameWithoutExtension(Object).IndexOf(".object")) + "</object>" + "@";
                    System.IO.File.Copy(Object, path + "projectA.gmx\\objects\\" + Path.GetFileName(Object));
                }
            }
            ...
            // ROOMS
            rewrite += "<rooms name=\"rooms\">@";

            //rewrite += "    <name>" + "rooms" + "</name>@";

            string[] rooms = Sounds.Get_Files(path + "rooms\\converted\\");
            List<string> actual_rooms = new List<string> { };
            foreach (string room in rooms)
            {
                if (Path.GetExtension(Path.GetFileName(room)) == ".gmx")
                {
                    actual_sounds.Add(Path.GetFileNameWithoutExtension(room));
                    rewrite += "<room>" + "rooms\\" + Path.GetFileNameWithoutExtension(room).Remove(Path.GetFileNameWithoutExtension(room).IndexOf(".room")) + "</room>" + "@";
                    System.IO.File.Copy(room, path + "projectA.gmx\\rooms\\" + Path.GetFileName(room));
                }
            }
            ...
            rewrite += "    <rtf>" + "help.rtf" + "</rtf>@";

            rewrite += "</help>@";

            // TUTORIALSTATE
            rewrite += "<TutorialState>@";

            rewrite += "    <isTutorial>" + "0" + "</isTutorial>@";
            rewrite += "    <TutorialName>" + "" + "</TutorialName>@";
            rewrite += "    <TutorialPage>" + "0" + "</TutorialPage>@";
            ...
            System.IO.File.OpenWrite(path + "projectA.gmx\\" + "help.rtf");
            var File = System.IO.File.OpenWrite(path + "projectA.gmx\\" + "projectA" + ".project.gmx");

            File.Close();

            System.IO.File.WriteAllText(path + "projectA.gmx\\" + "projectA" + ".project.gmx", rewrite);

            // OUTPUT
            Form.running = false;
            Form.log_text.Add("Finished project creation.");
        }
